...
function Project() {
  ...
  const extractLyricsJSON = (projectt: any) => {
    const firstIndex = projectt.choreography.indexOf("{")
    const lastIndex = projectt.choreography.lastIndexOf("}")
    const jsonString = projectt.choreography.substring(firstIndex, lastIndex+1);
    if (jsonString)
    {
      const lyricsJSON = JSON.parse(projectt.choreography.substring(firstIndex, lastIndex+1));
  ...
  const fetchProject = async () => {
    ...
    //   }
    // setProject(proj)
    extractLyricsJSON(res.data["project"]); 
    // hahaha
  ...
  {
    console.log("hereee ", stepSequence)
